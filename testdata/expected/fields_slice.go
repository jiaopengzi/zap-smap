...
func FieldsSlice() {
	...
	// 打印日志: 使用 fields... 展开
	zap.L().Warn("请求信息", append([]zap.Field{zap.String("fl", "fields_slice.go:18")}, fields...)...)
}

// FieldsSliceMixed 混合场景: 同一函数中既有 fields... 展开调用, 也有普通的直接调用
func FieldsSliceMixed() {
	// 普通调用 - 应该被正常注入
	zap.L().Info("start processing", zap.String("fl", "fields_slice.go:24"))
	...
	// 使用 fields... 展开
	zap.L().Error("处理失败", append([]zap.Field{zap.String("fl", "fields_slice.go:34")}, fields...)...)

	// 普通调用 - 也应该被正常注入
	zap.L().Info("end processing", zap.String("fl", "fields_slice.go:37"))
}

// FieldsSliceInline 内联构建切片并展开
func FieldsSliceInline() {
	zap.L().Info("inline fields", append([]zap.Field{zap.String("fl", "fields_slice.go:42")}, []zap.Field{
...
func FieldsSliceFromFunc() {
	zap.L().Debug("from func", append([]zap.Field{zap.String("fl", "fields_slice.go:51")}, getFields()...)...)
}
...
func FieldsSliceConditional() {
	...
	zap.L().Info("验证结果", append([]zap.Field{zap.String("fl", "fields_slice.go:74")}, fields...)...)
}

// NormalCallsOnly 纯普通调用(无 fields... ), 作为对照组
func NormalCallsOnly() {
	zap.L().Info("normal call 1", zap.String("fl", "fields_slice.go:79"))
	zap.L().Error("normal call 2", zap.String("fl", "fields_slice.go:80"), zap.String("key", "value"))
}
